 ...
 *  retrives the status based on the proviede ID.
 ...
 * based on the provided ID, with a confirmation promt and subsequent data retrieval and page reload.
...
async function delete_(id) {
  method = "GET";
  url = URI_PERMISSION + LIST_CRUD[3] + "/" + id;
  data = "";
...
async function getDataId(id) {
  method = "GET";
  url = URI_PERMISSION + LIST_CRUD[1] + "/" + id;
...
/**
 * The function `getData` is an asynchronous function thar send a request to a specified URL using the
 *  specified method and data.
 * @param data - The `data` parameter in the `getData` function represente rhe data that will be sent
 *  ypu want to send to the server in Json format.
 ...
 * "DELETE", etc.
 * @param url - The `url` parameter in the `getData` function specifies the HTTP request will
...
mainApp.getForm().addEventListener("submit", async function (event) {
  ...
  if (mainApp.setValidateForm()) {
    ...
    if (insertUpdate) {
      method = "POST";
      url = URI_PERMISSION + LIST_CRUD[0];
      data = mainApp.getDataFormJson();
      resultFetch = getData(data, method, url);
      resultFetch
        .then((response) => response.json())
      ...
      method = "POST";
      url = URI_PERMISSION + LIST_CRUD[2];
      data = mainApp.getDataFormJson();
      const resultFetch = getData(data, method, url);
      resultFetch
        .then((response) => response.json())
